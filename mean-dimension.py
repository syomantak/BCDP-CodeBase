...
# have a list of dimensions d = 5,10,15,20,25,30
dim = [5,10,15,20,30,40,50,60,75,90,120]
...
plt.plot(dim,[np.median(np.array(x)) for x in error_bdp],color='blue',label=r'$M_{\mathsf{mean}}(\cdot)$')
plt.fill_between(dim, [np.quantile(np.array(x),0.25) for x in error_bdp], [np.quantile(np.array(x),0.75) for x in error_bdp],
    alpha=0.2, facecolor='blue')

plt.plot(dim,[np.median(np.array(x)) for x in error_ldp],color='red',label=r'\boldmath$\min \delta$-LDP')
plt.fill_between(dim, [np.quantile(np.array(x),0.25) for x in error_ldp], [np.quantile(np.array(x),0.75) for x in error_ldp],
